...
CENSUS_API_KEY = "a31cae36f24172e67455cce4f8637d7e2fd87ac6"
...
print(counties_nj[["NAME", "COUNTYFP", "geometry"]].head(3).to_string(index=False))
...
nj_crosswalk["zip_code"] = nj_crosswalk["zip_code"].str.zfill(5)
valid_nj_zips = set(nj_crosswalk["zip_code"])

pre     = len(zcta_nj)
zcta_nj = zcta_nj[zcta_nj["zip"].astype(str).str.zfill(5).isin(valid_nj_zips)]
print(f"  Border ZIP filter: {pre} → {len(zcta_nj)} ZCTAs ({pre - len(zcta_nj)} dropped)")
print(f"  ✔ ZCTA boundaries loaded: {len(zcta_nj):,} ZCTAs")
print(zcta_nj[["zip", "geometry"]].head(3).to_string(index=False))
...
nj_zips = set(zcta_nj["zip"].astype(str).str.zfill(5).unique())
pre    = len(acs_df)
acs_df = acs_df[acs_df["zip"].str.zfill(5).isin(nj_zips)]
...
if not places_df.empty:
    ...
    if zip_col is None:
        ...
        places_df["zip"] = places_df["zip"].astype(str).str.zfill(5)
...
crosswalk_df["zip"] = crosswalk_df["zip"].str.zfill(5)
...
hud_zip_to_tract["zip"]   = hud_zip_to_tract["zip"].str.zfill(5)
...
osm_counts["zip"] = osm_counts["zip"].astype(str).str.zfill(5)

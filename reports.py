...
if BOOT_FILE.exists():
    ...
    print(f"\n  Bootstrap CV metrics (mean ± std):\n")

    # Only operate on numeric columns — drop any label/fold/string columns
    boot_numeric = boot.select_dtypes(include="number")

    if boot_numeric.empty:
        print("  No numeric columns found in bootstrap file — skipping.")
    elif len(boot_numeric) > 1:
        summary = pd.DataFrame({
            "metric": boot_numeric.columns,
            "mean"  : boot_numeric.mean().round(4).values,
            "std"   : boot_numeric.std().round(4).values,
            "min"   : boot_numeric.min().round(4).values,
            "max"   : boot_numeric.max().round(4).values,
        })
        print(summary.to_string(index=False))
        save(summary, "bootstrap_metrics_summary.csv")
    else:
        # Single row — just print it as-is
...
if SCV_FILE.exists():
    ...
    if len(scv) > 1:
        scv_summary = pd.DataFrame({
            "metric": scv.columns,
            "mean"  : scv.mean().round(4).values,
            "std"   : scv.std().round(4).values,
        })
        print(f"\n  Spatial CV mean ± std:\n")

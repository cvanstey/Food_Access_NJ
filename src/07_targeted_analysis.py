...
# =====================================================================
# 1. FORCE CORRELATION CLEANING *BEFORE* SPEARMAN CALCULATION
# =====================================================================
cols_to_correlate = [
    "pct_no_vehicle", "pct_poverty",
    "Coronary Heart Disease % (Adults)", "Diabetes % (Adults)",
    "Any Disability % (Adults)", "All Teeth Lost % (Adults 65+)",
    "High Cholesterol % (Adults)", "Stroke % (Adults)",
    "Depression % (Adults)", "Poor Physical Health % (Adults)"
]

# Ensure master DataFrame is clean and strictly numeric
df["pct_elderly"] = pd.to_numeric(df["pct_elderly"], errors='coerce')

for col in cols_to_correlate:
    if col in df.columns:
        if df[col].dtype == 'object':
            # Strip out any literal '%' or whitespace characters if present
            df[col] = df[col].astype(str).str.replace('%', '', regex=False).str.strip()

        # Coerce strings to real numeric floats, forcing errors to NaN
        df[col] = pd.to_numeric(df[col], errors='coerce')

# =====================================================================
# 2. RUN SPEARMAN LOOP WITH NAN POLICY
# =====================================================================
print(f"\n  Spearman correlations: % elderly × food access metrics:\n")

spear_rows = []
for col in [c for c in compare_cols + ELDERLY_HEALTH if c in df.columns]:
    # Pass entire series directly; scipy will automatically align and omit paired NaNs
    r, p = spearmanr(df["pct_elderly"], df[col], nan_policy='omit')

    # Handle edge case where a column might still be completely empty
    if pd.isna(r):
        sig_flag = "ns"
    else:
    ...
    spear_rows.append({
        "metric": col,
        "spearman_r": round(r, 3) if pd.notna(r) else "NaN",
        "p_value": round(p, 4) if pd.notna(p) else "NaN",
        "sig": sig_flag,
    })
...
# ── Save ─────────────────────────────────────────────────────────────────────
output_path = DATA / "targeted_analysis_report.docx"
